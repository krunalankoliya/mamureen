<?php
    ...
    // Auto-create maaraz table if missing
    mysqli_query($mysqli, "
    CREATE TABLE IF NOT EXISTS `maaraz` (
      `id`                   INT(11)      NOT NULL AUTO_INCREMENT,
      `ideal_model_scale`    TINYINT(3)   UNSIGNED DEFAULT NULL,
      `prep_hours`           DECIMAL(6,2) UNSIGNED DEFAULT NULL,
      `attendance_from`      DATE         DEFAULT NULL,
      `attendance_to`        DATE         DEFAULT NULL,
      `duration_per_day`     SMALLINT(5)  UNSIGNED DEFAULT NULL,
      `avg_visitor_minutes`  SMALLINT(5)  UNSIGNED DEFAULT NULL,
      `tafheem_text`         TEXT         DEFAULT NULL,
      `engagement_strategy`  TEXT         DEFAULT NULL,
      `key_takeaways`        TEXT         DEFAULT NULL,
      `feedback_impact`      VARCHAR(50)  DEFAULT NULL,
      `additional_info`      TEXT         DEFAULT NULL,
      `instruction`          TINYINT(1)   DEFAULT 0,
      `user_its`             BIGINT(8)    DEFAULT NULL,
      `jamaat`               VARCHAR(100) DEFAULT NULL,
      `added_its`            BIGINT(8)    DEFAULT NULL,
      `added_ts`             TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (`id`),
      KEY `idx_jamaat`   (`jamaat`),
      KEY `idx_added_ts` (`added_ts`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
");

    // check if current user already submitted for this mauze
    ...
    if (isset($_POST['submit_maaraz'])) {
        if ($already_filled) {
            $message = ['type' => 'danger', 'text' => 'You have already submitted the Ma\'raz report for this mauze on ' . date('d-M-Y H:i', strtotime($filled_on)) . '.'];
        } else {
    ...
    // recalc personal flag in case we just inserted
    if (! $already_filled) {
        $check_q2 = mysqli_query($mysqli, "SELECT added_ts FROM `maaraz` WHERE user_its = $user_its AND jamaat = '$mauze_esc' LIMIT 1");
        if ($check_q2 && $check_q2->num_rows) {
            $already_filled = true;
            $row2 = $check_q2->fetch_assoc();
            $filled_on = $row2['added_ts'];
        }
    }
