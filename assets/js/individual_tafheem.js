$(document).ready(function () {
  ...
  $(".view-btn").on("click", function () {
    ...
    $("#edit_target_info").text(
      $(this).data("name") + " (" + $(this).data("its") + ")",
    );
    ...
    $.ajax({
      ...
      success: function (reasons) {
        ...
        if (!reasons || reasons.length === 0) {
          $filterList.html(
            '<div class="ts-placeholder">No reasons found for this type</div>',
          );
  ...
  function updateLabel() {
    ...
    if (count === 0) {
      ...
      $filterLabel.html(
        count + ' reasons selected <span class="ts-badge">' + count + "</span>",
      );
    }
  }
