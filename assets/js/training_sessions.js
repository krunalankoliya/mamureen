$(document).ready(function () {
  ...
  $("#ts_select_all").on("click", function (e) {
    e.preventDefault();
    $filterList
      .find('.ts-item:not(.hidden) input[type="checkbox"]')
      .prop("checked", true);
    syncHiddenInputs();
    updateLabel();
  });
  ...
  $typeSelect.on("change", function () {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        $filterList.html(
          '<div class="ts-placeholder text-danger">Error ' +
            xhr.status +
            ": could not load titles</div>",
        );
  ...
  function updateLabel() {
    ...
    if (count === 0) {
      ...
      $filterLabel.html(
        count + ' titles selected <span class="ts-badge">' + count + "</span>",
      );
    }
  }
  ...
  if (uploadForm) {
    uploadForm.addEventListener("submit", function (e) {
      ...
      // Validate file sizes (5 MB each) and image types (JPEG/PNG only)
      ...
      for (var i = 0; i < fileInputs.length; i++) {
        if (fileInputs[i].files.length > 0) {
          ...
          if (
            ...
            allowedImgTypes.indexOf(file.type) === -1
          ) {
            alert(
              'Image "' +
                file.name +
      ...
      // Do not force a client-side timeout; allow slower multi-file uploads to finish.
      xhr.timeout = 0;
      ...
      xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          uploadMessage.textContent =
            "\u2714 Submitted successfully! Refreshing\u2026";
