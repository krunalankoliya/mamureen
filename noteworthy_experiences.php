...
// Fetch categories
$query = "SELECT *, CASE WHEN category_name_ld IS NOT NULL AND category_name_ld != '' THEN CONCAT(category_name, ' / ', category_name_ld) ELSE category_name END AS display_name FROM `bqi_categories` WHERE `category_type` = 'experience' AND `is_active` = 1 ORDER BY `sort_order`, `category_name`";
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="d-grid gap-2">
                                                    <button type="submit" name="submit_experience" class="btn btn-primary">Submit</button>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
...
<?php if (!empty($show_popup)) : ?>
    alert('Noteworthy experience submitted successfully!');
<?php endif; ?>
...
$(document).ready(function () {
    $('#datatable').DataTable({
        ...
        pageLength: 10,
        buttons: [
            { extend: 'copy', filename: function(){ return 'noteworthy_experiences-' + Date.now(); } },
            { extend: 'csv', filename: function(){ return 'noteworthy_experiences-' + Date.now(); } },
            { extend: 'excel', filename: function(){ return 'noteworthy_experiences-' + Date.now(); } }
        ]
    });
});
</script>
